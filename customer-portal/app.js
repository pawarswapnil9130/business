...
function renderProductsGrid() {
  ...
  grid.innerHTML = state.filteredProducts.map(p => {
    ...
    const imageHtml = p.imageUrl
      ? `<img src="${p.imageUrl}" alt="${p.name}" class="ecom-card-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="card-no-img" style="display:none;"><span class="card-icon">👔</span><span class="no-img-brand">${p.designBrand || 'CASA APPAREL'}</span></div>`
